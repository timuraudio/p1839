...
- Added cross-reference to [intro.object] p8 in wording.
...
With the current specification, the only way to access an object representation is to copy it into another object using `std::memcpy` or `std::bit_cast`. It should be noted that this only works for trivially copyable types, does not permit in-place modification, and does not work for arrays with dynamic extents. On the other hand, directly accessing object representations as proposed here works for any type with contiguous storage.
...
> [4]{.pnum} The object representation of an object nested within an object `o` is guaranteed to appear in the object representation of `o`.
:::
...
An operation that begins the lifetime of an array of char, unsigned char, or std​::​byte implicitly creates objects within the region of storage occupied by the array. [For the purposes of implicit object creation, an operation that begins the lifetime of an object `a` is not considered to begin the lifetime of the object representation of `a` unless `a` is an array of cv `unsigned char`.]{.add}
